...
router.post('/activities/:id/submissions', requireRole('student'), upload.single('file'), async (req, res) => {
  ...
  const submission = { id: nanoid(), activityId: req.params.id, userId: req.user!.id, fileName: req.file.originalname, filePath: req.file.path, mimeType: req.file.mimetype, note: (req.body.note as string) || '', status: 'pending' as const, createdAt: now(), updatedAt: now() };
